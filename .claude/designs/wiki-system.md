...
- [x] ~~Which GitHub library: `PyGithub` (higher-level) or raw `httpx` to GitHub REST API (fewer deps)?~~ **Deferred:** the choice is now isolated behind the `WikiRepo` Protocol. Pick when implementing `GithubWikiRepo`; callers are unaffected. See [0003-wiki-repo-protocol-abstraction](../decisions/0003-wiki-repo-protocol-abstraction.md).

## Decision Log

ADRs scoped to this design, append-only. See [.claude/decisions/](../decisions/README.md).

- [0001 — Consolidate entry-type taxonomy in `utils/wiki_layout.py`](../decisions/0001-wiki-layout-single-source-of-truth.md)
